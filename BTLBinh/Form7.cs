...
namespace BTLBinh
{
    ...
    FROM 
        SANPHAM s 
    LEFT JOIN 
        CHITIETHDN c ON s.MaSP = c.MaSP AND c.MaHDN = '{maHDN}' 
    WHERE 
        s.MaSP = '{maSP}'";
        ...
        public void LoadChiTiet(string maHDN)
        {
    ...
    SELECT 
        ...
        s.TenSP, 
        c.SoLuong, 
        s.GiaNhap AS DonGia,  -- Đơn giá từ bảng sản phẩm
        c.KhuyenMai, 
        (c.SoLuong * (s.GiaNhap - (s.GiaNhap * c.KhuyenMai / 100))) AS ThanhTien
    FROM 
        CHITIETHDN c 
    JOIN 
    ...
    WHERE 
        ...
        private decimal GetGiaSP(string maSP)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    gia = Convert.ToDecimal(dt.Rows[0]["GiaNhap"]); // Lấy giá sản phẩm từ kết quả truy vấn
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    LoadChiTiet(maHDN); // Tải lại chi tiết sau khi cập nhật
                }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (dgvDanhSach.CurrentRow != null)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    string deleteQuery = $"DELETE FROM CHITIETHDB WHERE MaHDB = '{maHDN}' AND MaSP = '{maSP}'";
                    try
                    {
        ...
        private void btnXuatExcel_Click(object sender, EventArgs e)
        {
            ...
            worksheet.Cells[1, 2] = maHDN; // Mã hóa đơn
            ...
            {
                FileName = "ChiTietHoaDonNhap.xlsx",
